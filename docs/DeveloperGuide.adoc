...
To begin, the user enters the command and the relevant arguments, AddressbookParser will call this command's parser, FindPersonsWithTagsCommandParser, which will then parse the arguments and eventually return the proper FindPersonsWithTagsCommand.

Within the command parser, the parser first checks if the arguments are empty.
...
[TIP]
It is therefore recommended that future developers improve this method by implementing a proper dictionary/library for this method, after weighing the costs and the benefits.

The parser then returns the proper FindPersonWithTagsCommand with the proper predicate (details in later section), created with the improved tag list, to be executed.

==== Implementation of Predicate
The predicate associated with this functionality is called PersonContainsTagsPredicate.
...
Within the test method, all of the person's tags, if any, are appended to a string.
