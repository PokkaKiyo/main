...
** `FrIeNdS` will match `friends`
* The order of the keywords does not matter.
** `classmates roommates` will match `roommates classmates`.
* Only full words will be matched.
** `col` will not match `colleagues`.
* Will attempt to be more lenient with singular and plural tense.
...
*** Searching by tag `glass` will attempt to find contacts with tags `glass` and `glassS` instead of `glasses`.
* Persons with at least one tag matching the keywords will be returned.
** `friends` will return a contact who has `friends` tags and more.
****
...
`coming in v1.4`
can support exclusion for certain tags
